 ...
 */
import { ATLAS_CELL_W, ATLAS_CELL_H, ATLAS_ASCENT, ATLAS_PIXELS, ATLAS_OFFSETS, ATLAS_WIDE_FLAGS, atlasRank, } from './atlas.js';
import { ATLAS_GRAY_CELL_W, ATLAS_GRAY_CELL_H, ATLAS_GRAY_ASCENT, ATLAS_GRAY_PIXELS, ATLAS_GRAY_OFFSETS, ATLAS_GRAY_WIDE_FLAGS, atlasGrayRank, } from './atlas-gray.js';
import { ATLAS_CELL_W as JBM10_CELL_W, ATLAS_CELL_H as JBM10_CELL_H, ATLAS_ASCENT as JBM10_ASCENT, ATLAS_PIXELS as JBM10_PIXELS, ATLAS_OFFSETS as JBM10_OFFSETS, ATLAS_WIDE_FLAGS as JBM10_WIDE_FLAGS, atlasRank as jbMono10Rank, } from './atlas-jbmono10.js';
import { ATLAS_GRAY_CELL_W as JBM10_GRAY_CELL_W, ATLAS_GRAY_CELL_H as JBM10_GRAY_CELL_H, ATLAS_GRAY_ASCENT as JBM10_GRAY_ASCENT, ATLAS_GRAY_PIXELS as JBM10_GRAY_PIXELS, ATLAS_GRAY_OFFSETS as JBM10_GRAY_OFFSETS, ATLAS_GRAY_WIDE_FLAGS as JBM10_GRAY_WIDE_FLAGS, atlasGrayRank as jbMono10GrayRank, } from './atlas-gray-jbmono10.js';
import { encodeGrayPng, encodeRgbPng } from './png.js';
export const DEFAULT_RENDER_FONT = 'spleen-5x8';
const DEFAULT_ATLAS = {
    bit: {
        cellW: ATLAS_CELL_W,
        cellH: ATLAS_CELL_H,
...
function grayGlyph(codepoint, font) {
    ...
    if (selected !== DEFAULT_ATLAS.gray) {
        ...
        if (fallbackRank >= 0)
            return { atlas: DEFAULT_ATLAS.gray, rank: fallbackRank };
    }
    return null;
}
...
export function renderCellWidth(style = {}) {
...
export function renderCellHeight(style = {}) {
    const atlas = style.aa === true ? atlasSet(style.font).gray : atlasSet(style.font).bit;
...
function cellsFor(codepoint, markerScale = 1, font = DEFAULT_RENDER_FONT) {
...
function classSlotForCodepoint(cp) {
    if (cp >= 0x30 && cp <= 0x39)
        return CLASS_SLOT_DIGIT; // 0-9
    if (cp >= 0x41 && cp <= 0x5a)
        return CLASS_SLOT_UPPER; // A-Z
    if (cp >= 0x61 && cp <= 0x7a)
        return CLASS_SLOT_LOWER; // a-z
    return CLASS_SLOT_OTHER;
}
/** 1px class micro-marks in cell margins (pre-invert ink = 255). */
function paintClassTick(fb, fbW, fbH, baseX, baseY, cellW, cellH, codepoint, colorMask, colorSlot) {
...
export function measureLineCols(line, markerScale = 1, font = DEFAULT_RENDER_FONT) {
    let w = 0;
    for (const ch of line)
        w += cellsFor(ch.codePointAt(0), markerScale, font);
    return w;
}
/** Always renders at full canvas width. Signature kept for transform.ts compatibility; returns cols unchanged. */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function shrinkColsToContent(text, cols, markerScale = 1, font = DEFAULT_RENDER_FONT) {
 ...
 */
export function measureContentCols(text, maxCols, markerScale = 1, font = DEFAULT_RENDER_FONT) {
...
export function wrapLines(text, cols, markerScale = 1, font = DEFAULT_RENDER_FONT) {
    const out = [];
 ...
 */
function blitGlyph(fb, fbW, x, y, codepoint, font = DEFAULT_RENDER_FONT, markerMask = null) {
    const glyph = bitGlyph(codepoint, font);
    if (!glyph)
        return 0;
 ...
 */
function blitGlyphGray(fb, fbW, x, y, codepoint, font = DEFAULT_RENDER_FONT) {
...
function blitGlyphScaled(fb, markerMask, fbW, fbH, x, y, codepoint, scaleX, font = DEFAULT_RENDER_FONT) {
    const glyph = bitGlyph(codepoint, font);
    if (!glyph)
        return 0;
    const { atlas, rank } = glyph;
...
function dilateInk(fb, fbW, fbH, radius, axis = 'both') {
...
export async function renderChunkToPng(text, cols = DEFAULT_COLS, style = {}, maxHeightPx = MAX_HEIGHT_PX, slotText) {
    ...
    const colorMask = (useColorCycle || useColorByRole || useColorByClass) ? new Uint8Array(width * height) : null;
    let droppedChars = 0;
